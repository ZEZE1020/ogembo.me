...
In a test environment, I might click around the Google Cloud Console to build this. But for this, we want a production mindset. We want reproducibility, version control, and resilient systems that recover automatically. We want manifests.
...
Let's update our deployment to include them.
...
The Pods are running, but they are isolated inside the cluster. You need to give users a stable entry point. A `LoadBalancer` service tells Google Cloud to provision a physical IP address that routes traffic to your Pods. While effective, for production GKE workloads, it's often better to use an `Ingress` controller, which can manage multiple services behind a single load balancer for better cost-efficiency and routing control.
...
After monitoring logs and metrics, you confirm the canary is stable. Now, it's time to promote it by updating the main deployment and removing the canary.
